...
- 工具栏的手形/鼠标指针按钮可切换触屏操作和电脑操作；手机默认触屏操作，电脑默认电脑操作，并会记住手动选择。
- 触屏操作下，只有轻触远程输入框才会调起手机输入法；普通点击和滚动不会弹出。若某些浏览器阻止自动弹出，可点击工具栏的键盘图标。
- 电脑操作下，触摸按下、移动和抬起会按远程鼠标操作发送，可用于拖拽和文本选择。
- 支持中文/英文输入、输入法组合文本、退格和回车。
- 点击工具栏的画面设置按钮，可选择自动适应，也可输入 320×240 到 3840×2160 的自定义分辨率。
- 画面设置中可在 25% 到 500% 之间调整页面缩放，支持加减按钮、直接输入百分比、恢复 100%，以及 `Ctrl +`、`Ctrl -`、`Ctrl 0` 和 `Ctrl+滚轮`。
- 标签栏支持新建、切换和关闭标签页；网页通过 `target=_blank` 打开的页面会显示为新标签并自动切换。
...
- 声音使用 Chromium 标签页捕获，只传输当前虚拟浏览器标签页的音频；捕获期间服务端扬声器不播放该标签页声音，其他应用的声音也不会传给客户端。
